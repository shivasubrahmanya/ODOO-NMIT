...
class DatabaseManager:
    ...
    @staticmethod
    def mark_notifications_read(notification_ids: list):
        """Mark notifications as read"""
        if not notification_ids:
            return
        placeholders = ','.join(['%s'] * len(notification_ids))
        query = f"UPDATE notifications SET is_read = TRUE WHERE id IN ({placeholders})"
        return execute_query(query, tuple(notification_ids))
    ...
    @staticmethod
    def create_comment(project_id: int, task_id: int, author_id: int, content: str, parent_comment_id: int = None):
        """Create a new comment"""
        query = """
        INSERT INTO comments (project_id, task_id, parent_comment_id, author_id, content) 
        VALUES (%s, %s, %s, %s, %s)
        """
        return execute_query(query, (project_id, task_id, parent_comment_id, author_id, content))
    
    @staticmethod
    def get_comments(project_id: int = None, task_id: int = None):
        """Get comments for project or task with threading"""
        if project_id and task_id:
            # Comments for specific task
            query = """
            SELECT c.*, u.name as author_name
            FROM comments c
            JOIN users u ON c.author_id = u.id
            WHERE c.project_id = %s AND c.task_id = %s
            ORDER BY c.created_at ASC
            """
            params = (project_id, task_id)
        elif project_id:
            # Comments for project (not task-specific)
            query = """
            SELECT c.*, u.name as author_name
            FROM comments c
            JOIN users u ON c.author_id = u.id
            WHERE c.project_id = %s AND c.task_id IS NULL
            ORDER BY c.created_at ASC
            """
            params = (project_id,)
        else:
    ...
    @staticmethod
    def check_project_member(project_id: int, user_id: int):
        """Check if user is a member of the project"""
        query = "SELECT 1 FROM project_members WHERE project_id = %s AND user_id = %s"
        result = execute_query(query, (project_id, user_id), fetch_one=True)
        return result is not None
    
    @staticmethod
    def get_project_members_with_user_info(project_id: int):
        """Get project members with full user info for notifications"""
        query = """
        SELECT pm.user_id, u.id, u.name, u.email, pm.role
        FROM project_members pm
        JOIN users u ON pm.user_id = u.id
        WHERE pm.project_id = %s
        """
